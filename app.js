...
// ============ NEWS (GDELT, best-effort) ============
async function fetchGdeltOnce(query){
  const url = `${GDELT_URL}?query=${encodeURIComponent('"' + query + '"')}&mode=artlist&format=json&maxrecords=6&sort=datedesc&timespan=3d`;
  const controller = new AbortController();
  ...
  if(!res.ok){
    const bodyText = await res.text().catch(() => '');
    throw new Error(`HTTP ${res.status} ${bodyText.slice(0,150)}`);
  }
  const raw = await res.text();
  let data;
  try{ data = JSON.parse(raw); }
  catch(parseErr){ throw new Error('resposta não era JSON: ' + raw.slice(0,150)); }
  return {articles: (data.articles || []).slice(0,5)};
}
...
async function getNews(query){
  try{
    ...
    // instabilidade conhecida da API pública do GDELT: uma segunda tentativa costuma resolver
    await new Promise(r => setTimeout(r, 1200));
    try{
      return await fetchGdeltOnce(query);
    }catch(secondErr){
      return {__error: (secondErr.name || 'Erro') + ': ' + (secondErr.message || 'desconhecido') + ' (após 2 tentativas)'};
    }
  }
}
...
async function openBriefing(target){
  ...
  getWeather(lat, lon).then(w => {
    ...
    if(w && !w.__error){
      ...
      const diag = w && w.__error ? w.__error : 'motivo desconhecido';
      el.innerHTML = `<h3><span class="ic"></span>Agora</h3><div class="muted-note">Clima indisponível no momento.<br><small style="opacity:.6;">diagnóstico: ${diag}</small></div>`;
    }
  });
  ...
  if(target.kind === 'place'){
    getNews(newsQuery).then(result => {
      ...
      if(!el) return;
      if(result.__error){
        el.innerHTML = `<h3><span class="ic"></span>Sinal de mídia</h3><div class="muted-note">Sinal de notícias indisponível no momento.<br><small style="opacity:.6;">diagnóstico: ${result.__error}</small></div>`;
