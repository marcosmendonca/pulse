...
async function getNews(query){
  try{
    ...
    if(!res.ok){
      const bodyText = await res.text().catch(() => '');
      return {__error: `HTTP ${res.status} ${bodyText.slice(0,150)}`};
    }
    const raw = await res.text();
    let data;
    try{ data = JSON.parse(raw); }
    catch(parseErr){ return {__error: 'resposta não era JSON: ' + raw.slice(0,150)}; }
...
async function openBriefing(target){
  ...
  content.innerHTML = `
    ...
    ${detailHtml}
    ${target.kind === 'place' ? `<div class="section" id="newsSection"><h3><span class="ic"></span>Sinal de mídia</h3><div class="muted-note">Consultando GDELT — pode levar até 15s…</div></div>` : ''}
  `;
