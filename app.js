...
async function getNews(query){
  try{
    const url = `${GDELT_URL}?query=${encodeURIComponent(query)}&mode=artlist&format=json&maxrecords=6&sort=hybridrel&timespan=3d`;
    const res = await fetch(url);
    if(!res.ok) throw new Error('bad status');
    const data = await res.json();
    return (data.articles || []).slice(0,5);
  }catch(e){
    return null; // signals "unavailable", handled by caller
  }
}
...
async function openBriefing(target){
  ...
  content.innerHTML = `
    ...
    ${detailHtml}
    ${target.kind === 'place' ? `<div class="section" id="newsSection"><h3><span class="ic"></span>Sinal de mídia</h3><div class="muted-note">Consultando GDELT…</div></div>` : ''}
  `;
  ...
  if(target.kind === 'place'){
    getNews(newsQuery).then(articles => {
      const el = document.getElementById('newsSection');
      if(!el) return;
      if(articles === null){
        el.innerHTML = `<h3><span class="ic"></span>Sinal de mídia</h3><div class="muted-note">Sinal de notícias indisponível no momento (limite da API pública do GDELT). Tente novamente em instantes.</div>`;
        ...
        el.innerHTML = `<h3><span class="ic"></span>Sinal de mídia (últimos 3 dias)</h3>` +
          articles.map(a => `<div class="news-item"><a href="${a.url}" target="_blank" rel="noopener">${a.title}</a>
